...
export async function dragByHandle(page: Page, handle: Locator, to: Locator) {
  ...
  // Each DndContext renders its own live region, so select by content rather
  // than position — the sidebar's and the task list's sit at opposite ends
  const announcement = page.locator('[role="status"]').filter({ hasText: 'Draggable item' }).first()
