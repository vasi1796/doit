      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 border-b border-gray-200">
          ...
          <input
            ...
            className="flex-1 text-[16px] py-3 outline-none bg-transparent text-text-primary placeholder:text-text-tertiary"
            aria-label="Search tasks"
          />
          <kbd className="hidden md:inline-block text-[11px] text-text-tertiary border border-gray-200 rounded px-1.5 py-0.5">
            ESC
          </kbd>
        </div>
...
function SearchResult({ task, isActive, onSelect }: { task: Task; isActive: boolean; onSelect: () => void }) {
  ...
  return (
    ...
    >
      {priorityColor && (
        <span className="absolute left-0 top-2.5 bottom-2.5 w-[3px] rounded-r-full" style={{ backgroundColor: priorityColor }} />
      )}

      <div className={`w-[18px] h-[18px] mt-0.5 rounded-full border-2 shrink-0 flex items-center justify-center ${
        task.is_completed ? 'bg-accent border-accent' : 'border-[#c7c7cc]'
      }`}>
        {task.is_completed && (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <InlineMarkdown
            text={task.title}
            className={`flex-1 text-[14px] leading-snug truncate ${task.is_completed ? 'line-through text-text-secondary' : 'text-text-primary'}`}
          />
          <div className="flex items-center gap-1.5 shrink-0">
            <PriorityFlag priority={task.priority} size={12} />
            {due && (
              <span className={`text-[11px] whitespace-nowrap ${due.overdue ? 'text-danger font-medium' : 'text-text-secondary'}`}>
                {due.text}
              </span>
            )}
          </div>
        ...
        {task.description && (
          <p className="text-[12px] text-text-secondary truncate mt-0.5">{task.description}</p>
        )}
