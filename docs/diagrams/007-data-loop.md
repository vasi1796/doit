...
|------|------|--------|----------|
| **Initial sync** | First app load (no cursor) | `GET /api/v1/tasks` (3 filtered calls) + `/lists` + `/labels` → read models (fallback: `GET /api/v1/snapshots`) | Bootstrapping IndexedDB with full state |
