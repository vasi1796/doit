...
| Delete Task                | Soft-delete to Trash. Tasks remain in Trash until restored. (A 30-day auto-purge worker was designed but has not been built.)                                                                                                                                                                                                                                                                                                                | P0       |
...
| iCal Feed       | Read-only iCalendar (.ics) feed served at `/ical/{token}/calendar.ics`. Generates VEVENT entries for tasks with due dates, subscribable in Apple Calendar and Google Calendar so tasks appear alongside regular events. Authenticated via per-user token in the URL (calendar apps do not support OAuth for feed subscriptions). Generated on request — no background worker.                                                                            | P1       |
...
| Trash View        | Soft-deleted tasks with restore. (Auto-purge after 30 days was designed but has not been built.) | P0       |
...
| Filtering & Sorting | Filter tasks by priority, due date, labels, and completion status. Sort by due date, priority, title, or creation date. Sort preference is persistent per view (stored in IndexedDB, device-local — never synced). Available on List, Inbox, Label, and Today views. | P1       |
...
| Real-Time Push     | WebSocket connection delivers thin `{"type":"sync"}` pings when another device writes — no event payloads travel over the socket; each ping triggers a normal `/sync` pull (see ADR-012). On disconnect (server restart, network change, iOS backgrounding): fall back to polling, attempt WebSocket reconnection with exponential backoff (1s -> 2s -> 4s, capped at 30s) plus jitter, and on successful reconnection perform a sync pull to catch events missed during the disconnect window.                                                                                                                | P1       |
| Conflict Resolution| CRDT-based merge: LWW-Register for scalar fields, plain add/remove rows for labels/list membership, fractional index for ordering.                                                                                                                                                                                                                                                                                                                                                                                                  | P1       |
| Sync Status        | Visual indicator showing sync state: synced, syncing, offline, conflict.                                                                                                                                                                                                                                                                                                                                                                                                                                             | P1       |
| Server Backup Sync | Per-aggregate server-side snapshots as insurance against Safari storage eviction. On each sync, changed aggregates (tasks, lists, labels) are snapshot to a server-side table keyed by `aggregate_id` + `user_id`. The client tracks a single HLC sync cursor (not per-aggregate versions); incremental catch-up happens through the event-based `/sync` pull, with snapshots used only for all-or-nothing rehydration. If local IndexedDB is evicted, all snapshots for the user are pulled and IndexedDB is rehydrated on next launch.                                                                                          | P0       |
...
| User Allowlist  | `ALLOWED_EMAILS` env var lists permitted Google email addresses — required at startup outside dev mode, enforced fail-closed at login. Unauthenticated/unlisted users rejected.                               | P0       |
...
**Database:** PostgreSQL 16 (event store + read models). Connection pooling via `pgxpool` — one pool per process, sized by `DB_MAX_OPEN_CONNS` with `DB_CONN_MAX_LIFETIME` to recycle connections.
...
**WebSockets:** `gorilla/websocket` — thin sync pings only, no event payloads (ADR-012)
...
**Markdown Editor:** CodeMirror 6 with live inline preview (Obsidian-style). Markdown is typed and stored as a raw string — the entire string is the CRDT unit.
...
**Service Worker:** Hand-written `public/sw.js` (cache-first app shell, network-first navigations, push handlers). No Background Sync (unsupported by Safari) -- sync is triggered on app foreground and on polling interval.
...
- Server publishes event to RabbitMQ via the Transactional Outbox (event + outbox row written in same PostgreSQL transaction; poller publishes with publisher confirms)
- The WS relay consumes the broadcast queue and sends a thin `{"type":"sync"}` ping to the user's other connected devices
- Receiving devices react to the ping with a `/sync` pull and merge the fetched events into their local IndexedDB
...
- **users** -- `id`, `google_id`, `email`, `name`, `avatar_url`, `created_at` (allowlisting is enforced at login from `ALLOWED_EMAILS` — there is deliberately no `allowed` column; see migration 011)
- **lists** -- `id`, `user_id`, `name`, `colour`, `icon`, `position` (fractional index), `created_at`, `updated_at`
- **tasks** -- `id`, `user_id`, `list_id`, `title` (inline markdown string), `description` (full markdown string), `priority` (0--3), `due_date`, `due_time`, `recurrence_rule`, `position` (fractional index), `is_completed`, `completed_at`, `is_deleted`, `deleted_at`, `created_at`, `updated_at`
- **labels** -- `id`, `user_id`, `name`, `colour`, `created_at`
- **task_labels** -- `task_id`, `label_id` (join table, plain membership rows)
- **subtasks** -- `id`, `task_id`, `title`, `is_completed`, `position`, `created_at`
- **user_config** -- `id`, `user_id`, `theme` (light/dark/system), `sidebar_collapsed`, `default_list_id`, `updated_at` (defined in migration 002 but currently unused — preferences are device-local in IndexedDB)
...
- **Set fields** (labels on a task): **plain add/remove rows** -- sync pulls deliver events in HLC order, so no tags or tombstones are needed. An OR-Set was specified here originally but never integrated; see the ADR-002 addendum for why it was removed and when this must be revisited.
...
RESTful API with JSON payloads. All `/api/*` endpoints require JWT authentication. Auth, health, and iCal feed endpoints use their own authentication mechanisms as noted below. The sync endpoint handles bidirectional CRDT state exchange.
...
| GET    | `/api/v1/snapshots`                 | All aggregate snapshots for the user (rehydration after eviction)    |
| WS     | `/api/v1/ws`                        | WebSocket carrying thin `{"type":"sync"}` pings (ADR-012)            |
| GET    | `/api/v1/push/vapid-key`            | VAPID public key for Web Push subscription                           |
...
| GET/POST/DELETE | `/api/v1/ical/token`       | iCal feed token status / issue / revoke                              |
...
| DELETE | `/api/v1/tasks/:id/labels/:lid`     | Remove label from task                                               |
...
| DELETE | `/api/v1/labels/:id`                | Delete label (removes it from all tasks)                             |
...
| GET    | `/healthz`                          | Liveness probe: process up, PostgreSQL reachable. Used by Docker health checks. No auth. |
| GET    | `/ical/{token}/calendar.ics`        | iCalendar feed of tasks with due dates (token-authenticated, no JWT) |
...
- **doit-api** -- Go binary, exposes REST + WebSocket on `:8080`, runs migrations on startup, includes the outbox poller
- **postgres** -- PostgreSQL 16, persistent volume, event store + read models
- **rabbitmq** -- RabbitMQ 3.13+ with management plugin, persistent queues, topic exchange for event distribution
- **caddy** -- Reverse proxy with automatic Let's Encrypt TLS. Serves PWA static assets and proxies `/api/*` to Go backend
- **web-build** -- one-shot container that builds the React frontend into the volume Caddy serves
- **worker** -- Go binary, consumes all events from RabbitMQ, updates read models (projections)
- **worker-recurring** -- Go binary, consumes from RabbitMQ, generates next occurrences for recurring tasks
- **worker-reminder** -- Go binary, morning digest + per-task due-time Web Push notifications
- **deployer** -- webhook sidecar: GitHub push to main -> git pull -> rebuild app services (idles if `DEPLOY_WEBHOOK_SECRET` is unset)
...
Structured JSON logging with zerolog across all services (container logs are size-capped via Docker's json-file rotation). RabbitMQ's management UI exposes queue depth, consumer utilisation, message rates, and dead-letter queue size. A Prometheus `/metrics` endpoint with Grafana golden-signal dashboards was designed but has not been built — there is currently no metrics handler.
...
- Plain text description field for tasks (markdown editor deferred to Phase 2, aligning with P1 priority; shipped as CodeMirror 6)
...
- Service worker for app shell caching (shipped hand-written rather than Workbox) -- no Background Sync (unsupported by Safari)
- CRDT implementation: LWW-Register for scalar fields (start here, simplest), OR-Set for labels (later removed in favour of plain membership rows — see ADR-002 addendum), fractional indexing for task ordering
...
- Markdown editor for task descriptions (P1 feature, raw markdown string stored as CRDT LWW-Register; shipped as CodeMirror 6)
...
- Trash auto-purge worker (30-day cleanup) — *not built*
- iCal feed worker (rebuilds `.ics` on task changes) — *shipped as on-request generation instead, no worker*
...
- Prometheus metrics + Grafana dashboard (four golden signals + RabbitMQ queue depth, consumer utilisation, dead-letter size) — *not built; see §6.3*
- Structured logging with zerolog
- Chaos testing: kill workers mid-processing, drop WebSocket connections, delay sync responses

**Done when:** You kill a worker mid-processing, verify the message is redelivered via RabbitMQ, and the idempotent handler processes it exactly once. Recurring tasks auto-generate on completion. The outbox poller reliably delivers events to RabbitMQ. (The Grafana and trash-purge criteria were dropped along with those features.)
...
| ------------------------------ | ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CRDT complexity                | Bugs in merge logic cause data loss or duplication                                               | Start with LWW-only (simplest CRDT). Add fractional index incrementally (the planned OR-Set was removed — see ADR-002 addendum). Extensive property-based testing.                                                                                                                               |
...
- MDN Web Docs -- Service worker strategies for PWA offline support
...
- CodeMirror 6 documentation -- editor state, decorations, live markdown preview
